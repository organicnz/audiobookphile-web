...
2. install dependencies: `cd audiobookshelf-client-react && bun install`
...
4. In the audiobookshelf server repo run with `bun dev` as usual. This will serve the NextJS app using HMR.
...
1. `bun run build` this NextJS app
2. Start the audiobookshelf server with `bun run start-dev` (uses the dev.js file w/ production)
