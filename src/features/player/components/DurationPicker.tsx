  ...
  return (
    <InputWrapper disabled={disabled} readOnly={readOnly} borderless={borderless} size={size} className={className}>
      <fieldset cy-id="duration-picker-wrapper" className={wrapperClass} onClick={handleWrapperClick}>
        ...
        <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }}  className={sepClass}  onClick={() => mRef.current?.focus()} onMouseDown={(e) => e.preventDefault()}>
          :
        </div>
        ...
        <div role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); } }}  className={sepClass}  onClick={() => sRef.current?.focus()} onMouseDown={(e) => e.preventDefault()}>
          :
        </div>
