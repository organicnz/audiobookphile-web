...
export async function POST(
  ...
  try {
    ...
    const { currentTime, duration } = body as {
      currentTime?: number
      duration?: number
    }
    ...
    if (upsertError) {
      console.error('[API Session Close] DB upsert failed:', upsertError.message)
      return NextResponse.json({ error: 'Failed to save final progress' }, { status: 500 })
    }

    return NextResponse.json({ success: true })

  } catch (error: any) {
    console.error('[API Session Close] Error:', error)
