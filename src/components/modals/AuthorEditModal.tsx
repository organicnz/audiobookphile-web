...
export default function AuthorEditModal({ isOpen, user, author, onClose }: AuthorEditModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} className="p-4 py-6" onClose={onClose}>
      <div className="h-full w-full flex">
        ...
        <div className="grow mb-2 px-2 pt-2">
          <div className="flex grow ">
            <TextInput className="w-full" placeholder={t('LabelImageURLFromTheWeb')}></TextInput>
            <Btn color="bg-success" className="ml-2 flex-shrink-0">
              {t('ButtonSubmit')}
            </Btn>
          </div>
          ...
          <div className="flex gap-3 sm:gap-4 justify-center pt-6">
            {user.permissions.delete && (
              <Btn color="bg-error" onClick={onClose}>
                {t('ButtonRemove')}
              </Btn>
            )}
            <div className="grow" />
            <Btn onClick={onClose}>{t('ButtonQuickMatch')}</Btn>
            <Btn color="bg-success" onClick={() => {}}>
              {t('ButtonSave')}
            </Btn>
          </div>
        </div>
