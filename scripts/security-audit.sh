...
DANGEROUS_XSS=$(grep -r -n -E 'dangerouslySetInnerHTML|eval\(|new Function\(' src/ 2>/dev/null || true)
if [ -n "$DANGEROUS_XSS" ]; then
    echo "❌ SECURITY AUDIT FAILED: Unsanitized HTML rendering or dynamic evaluation found:"
    echo "$DANGEROUS_XSS"
    exit 1
fi
